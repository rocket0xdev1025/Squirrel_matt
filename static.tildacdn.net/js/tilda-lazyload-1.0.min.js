function t_lazyload__init() {
  ...
  elAllRecs && "yes" === elAllRecs.getAttribute("data-tilda-imgoptimoff")
    ? (window.lazy_imgoptimoff = "yes")
    : (window.lazy_imgoptimoff = "");
  for (
    var elstoSkip = document.querySelectorAll(".t156 .t-img"), i = 0;
    i < elstoSkip.length;
    i++
  )
  ...
  Array.prototype.forEach.call(elstoRound, function (el) {
    ...
    setTimeout(function () {
      ...
      if (
        ...
        window.jQuery &&
          ($ = jQuery)(document).bind("slide.bs.carousel", function () {
            setTimeout(function () {
              t_lazyload_update();
            }, 500);
          }),
...
function t_lazyLoad__appendImgStatToArr(img, startTime) {
  if (void 0 !== navigator.sendBeacon) {
    ...
    if (imgSrc) {
      ...
      0 === imgSrc.indexOf("https://" + t_lazyload__getThumbDomainName()) &&
        (imgLoadingTime.th = "y"),
        0 === imgSrc.indexOf("https://static.tildacdn") &&
          (imgLoadingTime.st = "y"),
        (imgLoadingTime.th || imgLoadingTime.st) &&
...
function t_lazyload__ping(type) {
  ...
  if ("static" == type) {
    ...
    if (
      "object" == typeof cs &&
      "string" == typeof cs.src &&
      0 === cs.src.indexOf(domain)
    )
      return;
  ...
  (img.src = domain + "/pixel.png"),
    ...
    setTimeout(function () {
      if ("y" !== window["lazy_ok_" + type]) {
        (window["lazy_err_" + type] = "y"), console.log(type + " ping error");
        var els = document.querySelectorAll(".loading");
        Array.prototype.forEach.call(els, function (el) {
          var src = "";
          (src = el.lazy_loading_src),
            "string" == typeof str &&
              0 === src.indexOf(domain) &&
              (el.classList.remove("loading"), (el.wasProcessed = !1));
        }),
          t_lazyload_update();
      }
    }, 1e4);
...
!(function (root, factory) {
  ...
  var _defaultSettings,
    ...
    _supportsObserver = !1,
    _staticUrlRegex = /\/static\.tildacdn\.(info|.{1,3})\//,
    _staticUrlReplaces = {};
  function _init() {
    _isInitialized ||
      ((_defaultSettings = {
  ...
  function _merge_objects(obj1, obj2) {
    ...
    for (propertyName in obj2)
      Object.prototype.hasOwnProperty.call(obj2, propertyName) &&
  ...
  function _convertToArray(nodeSet) {
    ...
    try {
      ...
      var array = [],
        i,
        l = nodeSet.length;
      for (i = 0; i < l; i++) array.push(nodeSet[i]);
      elsArray = array;
    }
    return (
      elsArray.forEach(function (element) {
        element.isSkipByPosition =
          ...
          !1 === _isExist(_getParent(element, "t396__carrier-wrapper")) &&
          "fixed" !== element.getAttribute("data-content-cover-parallax");
        var elRec = _getParent(element, "t-rec");
        _isExist(elRec) &&
          (element.isNotUnderScreenRange =
            !1 === elRec.hasAttribute("data-screen-max") &&
            !1 === elRec.hasAttribute("data-screen-min"));
      }),
      elsArray
    );
  }
  ...
  function _getParent(element, className) {
    for (var p = element.parentNode; p && p !== document; ) {
      if (!0 === _hasClass(p, className)) return p;
      p = p.parentNode;
    }
    return null;
  }
  ...
  function _setSources(target, source, srcsetDataAttribute, srcDataAttribute) {
    ...
    if (src) {
      ...
      if (
        ...
        (wrp = _getParent(source, "t-slds__wrapper")) ||
          (wrp = _getParent(source, "t-slds__container")),
          !1 === _isExist(wrp) &&
            (wrp = _getParent(source, "t-slds__thumbsbullet")),
          _isExist(wrp) &&
            ((width = wrp.clientWidth), (height = wrp.clientHeight));
      ...
      var x = "",
        ...
        foo;
      if (
        ("yes" == window.lazy_imgoptimoff && (doo = !1),
        ("y" !== window.lazy_err_thumb && "y" !== window.lazy_err_static) ||
          (doo = !1),
        "IMG" === source.tagName)
      )
        comm = "resize";
        ...
        var sourceStyles = getComputedStyle(source),
          bgPos = sourceStyles.backgroundPosition,
          foo;
        if (bgPos)
          "50%" == (x = (foo = bgPos.split(" "))[0])
            ? (x = "center")
            : "0%" == x
            ? (x = "left")
            : "100%" == x && (x = "right"),
            "50%" == (y = foo[1])
              ...
              : "100%" == y && (y = "bottom");
        (comm =
          "contain" == (bgsize = sourceStyles.backgroundSize)
            ? "contain"
            : "cover"),
      ...
      if (
        ...
        !0 === doo &&
          (src =
            ...
            height > 1e3 ||
            0 == width ||
            ("IMG" != source.tagName && 0 == height)
              ? t_lazyload__getWebPUrl(src)
              : t_lazyload__getResizeUrl(
                  source.tagName,
                  comm,
                  src,
                  width,
        ...
        "y" === window.lazy_err_static &&
          _isStaticUrl(src) &&
          (src = src.replace(_staticUrlRegex, "/static3.tildacdn.com/")),
        src)
      ) {
        if ("IMG" === target.tagName) target.setAttribute("src", src);
        else if ("IFRAME" === target.tagName) target.setAttribute("src", src);
        else if ("OBJECT" === target.tagName) target.setAttribute("data", src);
        else {
          var gradient, split;
    ...
    } else _removeClass(source, "loading");
  }
  ...
  function t_lazyload__round(comm, width, height, round) {
    if ("cover" == comm && width > 0 && height > 0) {
      ...
      rr <= 1
        ? (rr <= 0.8 && (ratio = 0.8),
          rr <= 0.751 && (ratio = 0.75),
          rr <= 0.667 && (ratio = 0.667),
          rr <= 0.563 && (ratio = 0.562),
          rr <= 0.501 && (ratio = 0.5),
    ...
    } else width > 0 && (width = Math.ceil(width / round) * round), height > 0 && (height = Math.ceil(height / round) * round);
    return [width, height];
  }
  function t_lazyload__getResizeUrl(tagName, comm, str, width, height, x, y) {
    ...
    if (0 == width && 0 == height) return str;
    if ("y" == window.lazy_err_thumb) return str;
    ...
    if ("resize" == comm) {
      var arrr;
      (arrr = str.split("/")).splice(
        str.split("/").length - 1,
        0,
        "-/resize/" +
          width +
          "x" +
          ("DIV" == tagName && height > 0 ? height : "") +
          "/" +
  ...
  function t_lazyload__getWebPUrl(str) {
    ...
    if (str.indexOf("/-/") > -1) return str;
    if (str.indexOf("lib") > -1) return str;
    if ("y" !== window.lazy_webp) return str;
    if ("y" === window.lazy_err_thumb) return str;
    var arrr = str.split("/"),
      newstr;
    return (
  ...
  function t_lazyload__reloadonError(element, src, startTime) {
    if ("string" == typeof src && null != src && "" != src) {
      var newsrc;
      if ((console.log("lazy loading err"), _isStaticUrl(src)))
        return (
          (window.lazy_err_static = "y"),
          void t_lazyload__reloadonError__reload(
            element,
            (newsrc = src.replace(_staticUrlRegex, "/static3.tildacdn.com/"))
          )
        );
      if (
        -1 !== src.indexOf(t_lazyload__getThumbDomainName()) &&
        -1 !== src.indexOf("/-/")
      ) {
        ...
        if (arrr.length > 3)
          for (var i = 0; i < arrr.length; i++)
            "" !== arrr[i] &&
              ("til" == arrr[i].substring(0, 3) &&
                36 == arrr[i].length &&
                4 == (arrr[i].match(/-/g) || []).length &&
                (uid = arrr[i]),
              i == arrr.length - 1 && (name = arrr[i]));
        if ("" !== uid && "" !== name)
          t_lazyload__reloadonError__reload(
            element,
            (newsrc = "/" + uid + "/" + name)
          );
  ...
  function t_lazyload__reloadonError__reload(element, src) {
    console.log("try reload: " + src),
      "IMG" === element.tagName
        ...
        : src && (element.style.backgroundImage = "url(" + src + ")");
  }
  ...
  return (
    (LazyLoad.prototype._showOnLoad = function (element) {
      ...
      !element.getAttribute("src") &&
        ...
        (fakeImg = document.createElement("img")).addEventListener(
          "load",
          loadCallback
        ),
        fakeImg.addEventListener("error", function (ev) {
          _removeClass(element, settings.class_loading),
            ...
            void 0 !== ev.path
              ? t_lazyload__reloadonError(
                  element,
                  ev.path[0].currentSrc,
                  startTime
                )
              : void 0 !== ev.target &&
                t_lazyload__reloadonError(
    ...
    (LazyLoad.prototype._showOnAppear = function (element) {
      ...
      ("IMG" !== element.tagName && "IFRAME" !== element.tagName) ||
    ...
    (LazyLoad.prototype._show = function (element) {
      this._settings.show_while_loading
          ...
          entries.forEach(function (entry) {
            ...
            if (
              ...
              var isAboveViewport = entry.boundingClientRect.top + scrollY < 0,
                ...
                isAboveInnerViewport =
                  parentZeroElem && 0 === parentZeroElem.style.top.indexOf("-");
              if (element.wasProcessed)
                return (
                  self._intersectionObserver.unobserve(element),
                  void (
                    self._settings.callback_processed &&
              ...
              (entry.isIntersecting ||
                isAboveViewport ||
                isAboveInnerViewport) &&
                (self._show(element), (element.wasProcessed = !0));
            }
          });
        },
    ...
    (LazyLoad.prototype.loadAnimatedImages = function () {
      ...
      function getTriggerElementOffset(element, type) {
        ...
        } else "viewport" === type && (trgEl = _getParent(element, "t396"));
      ...
      for (i = 0; i < elementsLength; i++)
        if (
          _hasClass((element = elements[i]), "tn-atom__img") ||
          _hasClass(element, "tn-atom")
        ) {
          var elContainer = _getParent(element, "tn-elem"),
            isAnimated = elContainer.getAttribute("data-animate-sbs-opts"),
            animEvent = elContainer.getAttribute("data-animate-sbs-event"),
            animTrgEls,
            animType;
          ("intoview" !== animEvent && "blockintoview" !== animEvent) ||
            (animType = "viewport"),
            elContainer.getAttribute("data-animate-sbs-trgels") ||
              (animType = "trigger"),
            (settings.skip_invisible && null === element.offsetParent) ||
              !isAnimated ||
              (isFarAway(elContainer, animType) &&
                (settings.show_while_loading
                  ? this._showOnAppear(element)
                  : this._showOnLoad(element),
                (element.wasProcessed = !0),
                settings.callback_processed &&
    ...
    if (null != elem) {
      ...
      if ("IMG" == tagName)
        ...
        var elemStyles = getComputedStyle(elem),
          str = elemStyles.backgroundImage
            .replace("url(", "")
            .replace(")", "")
            .replace(/"/gi, "");
        if ("contain" === elemStyles.backgroundSize) var prefix = "-/contain/";
        else var prefix = "-/cover/";
      } else var prefix = "-/cover/";
      if (
        !(
          null == str ||
          -1 === str.indexOf(prefix) ||
          str.indexOf(".svg") > 0 ||
          str.indexOf(".gif") > 0 ||
          str.indexOf(".ico") > 0 ||
          -1 === str.indexOf(t_lazyload__getThumbDomainName()) ||
